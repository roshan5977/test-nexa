...
public class ExamAdminServiceImplementation implements ExamAdminService {
    ...
    public ExamAdmin updateExamAdmin(ExamAdmin examAdmin) {
        if (examAdmin == null || examAdmin.getExam_admin_id() == null) {
            throw new IllegalArgumentException("Exam admin or ID cannot be null");
        }

        Optional<ExamAdmin> existingExamAdmin = examAdminRepository.findById(examAdmin.getExam_admin_id());

        if (!existingExamAdmin.isPresent()) {
            throw new ExamAdminNotFoundException("Exam admin not found with ID: " + examAdmin.getExam_admin_id());
        }
